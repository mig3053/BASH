...
if [[ -n $purge_after_input ]]; then
  ...
  echo "`date +%m%d%y`::Snapshots taken by $app_name will be eligible for purging after the following date (the purge after date given in seconds from epoch): $purge_after_date_fe."
fi
...
if $purge_snapshots; then
  echo "`date +%m%d%y`::Snapshot Purging is Starting Now."
